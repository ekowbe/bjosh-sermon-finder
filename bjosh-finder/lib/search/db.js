...
export function sql() {
  ...
  _sql = postgres(url, { prepare: false, max: 4, idle_timeout: 20 });
  return _sql;
}
...
export async function upsertSermon(s) {
  ...
  const rows = await db`
    insert into sermons
      (source, external_id, title, topics, scriptures, key_scripture, summary, audio_id, channel, published, content_hash)
    values
      (${s.source}, ${s.external_id}, ${s.title}, ${db.array(s.topics || [])}, ${db.array(s.scriptures || [])},
       ${s.key_scripture || ''}, ${s.summary || ''}, ${s.audio_id || ''}, ${s.channel || ''}, ${s.published || ''}, ${s.content_hash})
    on conflict (source, external_id) do update set
      ...
      channel = excluded.channel, published = excluded.published, content_hash = excluded.content_hash,
      indexed_at = now()
...
export async function hybridSearch(queryEmbedding, queryText, { k = 15, pool = 60 } = {}) {
  ...
  return db`
    ...
    select s.id as sermon_id, s.source, s.external_id, s.title, s.key_scripture, s.summary,
           s.audio_id, b.text as best_chunk, b.score
